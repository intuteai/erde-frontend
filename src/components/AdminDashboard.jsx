...
/** Table columns — only the requested three are sortable */
const columns = [
  ...
  { key: 'vcuId', label: 'VCU ID', width: 'w-36', sortable: false },
  { key: 'hmiId', label: 'HMI ID', width: 'w-36', sortable: false },
  { key: 'delivery', label: 'Date of Delivery', width: 'w-40', sortable: false },
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/vehicles-master`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      const mapped = res.data.map(row => ({
        id: row.vehicle_master_id,
        vehicleType: `${row.make || ''} ${row.model || ''}`.trim() || '—',
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      data = data.filter(r =>
        [r.customer, r.vehicleType, r.vehicleNo]
          .filter(Boolean)
  ...
  const handleCreate = async () => {
    ...
    if (!vehicle_unique_id || !customer_id || !vtype_id) {
      setError('Required fields: Unique ID, Customer ID, Vehicle Type ID');
      return;
    }

    try {
      await axios.post(
        `${API_BASE_URL}/api/vehicles-master`,
        { vehicle_unique_id, customer_id, vtype_id, vehicle_reg_no },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      await fetchData();
  ...
  const handleUpdate = async (id) => {
    const vehicle_reg_no = prompt('Enter new Vehicle Reg No:');
    if (!vehicle_reg_no) return;

    try {
      await axios.put(
        `${API_BASE_URL}/api/vehicles-master/${id}`,
        { vehicle_reg_no },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      await fetchData();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/vehicles-master/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      await fetchData();
      setError('Vehicle deleted.');
    } catch (e) {
      setError(e.response?.data?.error || 'Cannot delete: has live data');
    }
  };
                  ...
                  {columns.map((c) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {c.sortable && (
                          <ArrowUpDown
                            className={`w-4 h-4 ${sort.key === c.key ? 'opacity-100' : 'opacity-40'}`}
                          />
                        )}
                      </button>
                    </th>
                ...
                {loading ? (
                  ...
                  paged.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-gray-100">{row.vehicleNo}</td>
                      <td className="px-5 py-4 text-gray-100">{row.customer}</td>
                      <td className="px-5 py-4 text-gray-100">{row.vcuId}</td>
                      <td className="px-5 py-4 text-gray-100">{row.hmiId}</td>
